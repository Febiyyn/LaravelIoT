...
<body>
    <main>
        <section id="container">
            <div class="row">
                ...
                <div class="card">
                    <h3>Display LCD</h3>
                    <p class="text-lcd" id="lcd">.</p>
                    {{-- <input type="text" name="text-lcd" id="input-lcd"> --}}
    ...
    <script>
        ...
        const host = "wss://cobalaravel.cloud.shiftr.io:443/mqtt"
        ...
        const options = {
            ...
            protocolId: 'MQTT',
            protocolVersion: 4,
            username: 'cobalaravel',
            password: '8dlEnIJsZCG7au6V',
            clean: true,
        ...
        client.on("message", (topic, message) => {
            ...
            function updateLCD() {
                const suhu = document.getElementById('suhu').textContent.trim();
                const kelembapan = document.getElementById('kelembapan').textContent.trim();

                if (suhu !== "°C" && kelembapan !== "%") {
                // Format pesan untuk LCD
                const lcdMessage = `Suhu: ${suhu}, Kelembapan: ${kelembapan}`;

                // Kirim pesan ke topik "nusabot/lcd"
                client.publish("nusabot/lcd", lcdMessage, { qos: 1, retain: true });
            }
            // if(topic === "nusabot/lcd"){
